...
public interface ClusterServiceSnapshotDao extends CrudRepository<ClusterServiceShapshotEntity, Long> {
    ...
    @Query("select csse from ClusterServiceShapshotEntity csse left join csse.clusterSnapshotEntity cse left join csse.clusterServiceEntity csere where cse.id=?1")
